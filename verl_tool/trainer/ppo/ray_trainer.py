...
class AgentRayPPOTrainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        for test_data in tqdm(self.val_dataloader, desc="Validating Progress"):
            ...
            if tool_interact_info:
                for tool_interact in tool_interact_info:
                    # tool_interact can be a list of turns or a single dict
                    if isinstance(tool_interact, list):
                        for turn_tool_interact in tool_interact:
                            if "image" in turn_tool_interact:
                                if isinstance(turn_tool_interact['image'], list):
                                    turn_tool_interact['image'] = [x[:50] for x in turn_tool_interact['image']]  # crop the image to first 50 characters
                                elif isinstance(turn_tool_interact['image'], str):
                                    turn_tool_interact['image'] = turn_tool_interact['image'][:50] # for debug
                    elif isinstance(tool_interact, dict):
                        # Handle case where tool_interact is directly a dict (single turn)
        ...
        sample_inputs = [re.sub(r'<\|vision_start\|>.*?<\|vision_end\|>', '<image>', x, flags=re.DOTALL) for x in sample_inputs]
        sample_outputs = [re.sub(r'<\|vision_start\|>.*?<\|vision_end\|>', '<image>', x, flags=re.DOTALL) for x in sample_outputs]

        # concatenate data_sources before dumping
        data_sources = np.concatenate(data_source_lst, axis=0) if len(data_source_lst) > 0 else np.array([])
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            inputs = [re.sub(r'<\|vision_start\|>.*?<\|vision_end\|>', '<image>', x, flags=re.DOTALL) for x in inputs]
            outputs = [re.sub(r'<\|vision_start\|>.*?<\|vision_end\|>', '<image>', x, flags=re.DOTALL) for x in outputs]
            self._dump_generations(
                inputs=inputs,
                outputs=outputs,
                gts=sample_gts,
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if reward_extra_infos_dict:
                            ...
                            to_remove_keys = []
                            for k, v in reward_extra_infos_dict.items():
                                # Skip non-numeric fields (like 'ability' which is string)
