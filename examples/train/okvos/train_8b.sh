...
disable_mm_cache=True # set True if encounter mm_hash missing
...
model_pretty_name=$(basename $model_name | tr '[:upper:]' '[:lower:]')
run_name_postfix="grpo"
timestamp=$(date +"%Y%m%d_%H%M%S")
if [ "$enable_agent" = "True" ]; then
    run_name="${timestamp}-${reward_manager}-agent-${model_pretty_name}-n${n}-b${batch_size}-${ppo_mini_batch_size}-t${temperature}-${run_name_postfix}"
else
    run_name="${timestamp}-${reward_manager}-${model_pretty_name}-n${n}-b${batch_size}-${ppo_mini_batch_size}-t${temperature}-${run_name_postfix}"
fi
...
python -m verl_tool.servers.serve --host $host --port $port --tool_type "local_retrieval,keyframe" --workers_per_tool 8 &
server_pid=$!
