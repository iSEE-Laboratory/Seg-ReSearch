...
from training.sam2.sam2.modeling.sam2_base import SAM2Base
...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        train_memory_attention_only=False,
        **kwargs,
    ):
        ...
        if train_memory_attention_only:
            self.image_encoder.eval()
            for n, p in self.named_parameters():
                if 'memory_attention' not in n:
                    p.requires_grad = False
        
        if self.num_maskmem > 7:
            self.maskmem_tpos_enc.requires_grad = True

    def forward(self, input: BatchedVideoDatapoint):
        if self.training or not self.forward_backbone_per_frame_for_eval:
            # precompute image features on all frames before tracking
            # Check if the input batch is too large (e.g., more than 48 images), Process the batch in multiple smaller splits
            # import pdb; pdb.set_trace()
            if input.flat_img_batch.shape[0] > 32:
                img_chunks = torch.chunk(input.flat_img_batch, chunks=4, dim=0)
                ...
                for img_batch in img_chunks:
                    _backbone_out = self.forward_image(img_batch)

                    backbone_out["vision_features"].append(_backbone_out["vision_features"])
                    for j in range(3):
                        backbone_out["vision_pos_enc"][j].append(_backbone_out["vision_pos_enc"][j])
                        backbone_out["backbone_fpn"][j].append(_backbone_out["backbone_fpn"][j])

                backbone_out["vision_features"] = torch.cat(backbone_out["vision_features"], dim=0)
                backbone_out["vision_pos_enc"] = [
                    torch.cat(backbone_out["vision_pos_enc"][j], dim=0) for j in range(3)
                ]
                backbone_out["backbone_fpn"] = [
                    torch.cat(backbone_out["backbone_fpn"][j], dim=0) for j in range(3)
                ]
            else:
