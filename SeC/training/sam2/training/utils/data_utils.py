...
class BatchedVideoDatapoint:
    ...
    @property
    def flat_img_path(self) -> List[str]:
        """
        Returns a flattened img_path of shape [(B*T)]
        """
        return [img for video in self.img_path for img in video]
...
def collate_fn(
    ...
    """
    img_batch = []
    img_path = [[] for _ in range(len(batch))]
    for video in batch:
    ...
    for video_idx, video in enumerate(batch):
        ...
        for t, frame in enumerate(video.frames):
            img_path[video_idx].append(frame.image_path)
